...
// ═══════════════════════════════════════════════════════════════════════════════
// KEYFRAMES — explicit CW / CCW so there is zero ambiguity
// ═══════════════════════════════════════════════════════════════════════════════
const KEYFRAMES = `
  /* Logo gentle float */
...
export default function SignUp() {
  ...
  const submit = async () => {
    ...
    setTimeout(() => navigate("/scan"), 1700);
  };

  return (
    <>
      {/* ── FULL ANIMATION STYLESHEET ── */}
      ...
      <div style={{
        ...
        <nav style={{
          ...
          {/* ── FLOATING SEAL: position left:55px top:-4px, animate-float ── */}
          <div
            onClick={() => navigate("/")}
            style={{
              position:"absolute",
        ...
        <main style={{
          ...
          <div style={{
            ...
            background:"radial-gradient(ellipse at center,rgba(63,125,88,.09) 0%,transparent 65%)",
          }}/>
          ...
          <div style={{
            ...
            background:"radial-gradient(ellipse at center,rgba(230,211,173,.13) 0%,transparent 65%)",
          }}/>

          {/* ── UPPER-LEFT SATELLITE GLOBE: same style as left satellite ── */}
          <img
            src={globeHero} alt="" aria-hidden="true"
            style={{
              position:"absolute",
              left:"0px", top:"2%",
              ...
              opacity:0.08,
              filter:
                "blur(2px) brightness(0.5) contrast(1.3) saturate(1.2) " +
                "drop-shadow(0 0 30px rgba(63,125,88,.4))",
              animation:"rotate-ccw 20s linear infinite",
              animationDelay:"-5s",
            }}
          />

          {/* ── LOWER-RIGHT SATELLITE GLOBE with orbital arc ── */}
          <div style={{
            position:"absolute",
            ...
            {/* Orbital arc ring — thin emerald stroke */}
            <svg
            ...
            >
              <circle
                cx="170" cy="170" r="162"
                fill="none"
                stroke="rgba(63,125,88,0.2)"
                strokeWidth="1"
                strokeDasharray="320 700"
                strokeDashoffset="0"
                strokeLinecap="round"
              />
              <circle
            ...
            <img
              ...
              style={{
                ...
                opacity:0.08,
                filter:
                  "blur(3px) brightness(0.5) contrast(1.3) saturate(1.2) " +
                  "drop-shadow(0 0 30px rgba(63,125,88,.4))",
                animation:"rotate-ccw 20s linear infinite",
                animationDelay:"-12s",
              }}
          ...
          {/* Aura ring — breathes independently */}
          <div style={{
            position:"absolute", top:"50%", left:"50%",
            width:"1100px", height:"1100px", borderRadius:"50%",
            background:"radial-gradient(circle,rgba(63,125,88,.12) 0%,rgba(63,125,88,.04) 45%,transparent 70%)",
            pointerEvents:"none", zIndex:0,
            animation:"aura-breathe 6s ease-in-out infinite",
          }}/>

          {/* ── CENTER GLOBE: 900px, opacity .1, CLOCKWISE 30s ── */}
          <img
            src={globeHero} alt="" aria-hidden="true"
            style={{
              position:"absolute", top:"50%", left:"50%",
              ...
              opacity:0.25,
              filter:
                "brightness(0.6) contrast(1.2) saturate(1.4) " +
                "drop-shadow(0 0 40px rgba(63,125,88,.6)) " +
                "drop-shadow(0 0 100px rgba(63,125,88,.3))",
              animation:"rotate-cw 30s linear infinite",
            }}
          />

          {/* ── LEFT SATELLITE: 300px, left 10%, top 50%, opacity .05,
                  blur 2px, COUNTER-CLOCKWISE 20s ── */}
          <img
            src={globeHero} alt="" aria-hidden="true"
            style={{
              position:"absolute",
          ...
          {/* ── RIGHT SATELLITE: 250px, right 12%, top 45%, opacity .04,
                  blur 3px, COUNTER-CLOCKWISE 20s (offset phase) ── */}
          <img
            src={globeHero} alt="" aria-hidden="true"
            style={{
              position:"absolute",
          ...
          {/* ════════════════════════════════════════
              FADED GLASS FORM: bg .08, blur 65px
          ════════════════════════════════════════ */}
          <div style={{
            ...
            <div style={{ textAlign:"center", marginBottom:"42px" }}>
              {/* logo1 wordmark + "Network" as unified brand lockup */}
              <div style={{
              ...
              <h1 style={{
                fontFamily:"var(--font-display)",
                fontSize:"clamp(2rem,4.5vw,2.4rem)",
            ...
            {done ? (
              <div style={{
                ...
                padding:"8px 0 12px",
                animation:"success-bloom .6s cubic-bezier(.34,1.56,.64,1) both",
              }}>
                <div style={{
                  ...
                  display:"flex", alignItems:"center", justifyContent:"center",
                  boxShadow:"0 0 40px rgba(63,125,88,.35), 0 0 80px rgba(63,125,88,.12)",
                }}>
                ...
                <div style={{ textAlign:"center" }}>
                  <p style={{
                    ...
                    fontWeight:800, color:"var(--color-dark)", margin:"0 0 8px",
                    letterSpacing:"-.02em",
                  }}>You're in.</p>
                  <p style={{
                    fontFamily:"var(--font-body)", fontSize:".85rem",
                    color:"var(--color-text)", margin:0, opacity:.7,
                  }}>Launching your first scan…</p>
                </div>
              </div>
            ) : (
              <>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:"26px", marginBottom:"36px" }}>
                  <div>
                    <Field label="Full Name" icon={<User/>} value={name}
                      onChange={e=>setName(e.target.value)} delay=".06s"/>
                    <Err msg={errors.name}/>
                  </div>
                  <div>
                    <Field label="Email Address" icon={<Mail/>} type="email" value={email}
                      onChange={e=>setEmail(e.target.value)} delay=".14s"/>
                  ...
                  <div>
                    <Field
                      ...
                      suffix={
                        <button type="button" onClick={()=>setShowPw(v=>!v)} style={{
                          ...
                          padding:"0 0 0 8px", color:"var(--color-text)",
                          opacity:.45, display:"flex", alignItems:"center",
                    ...
                    {pw && (
                      <div style={{ marginTop:"11px", animation:"field-rise .3s ease both" }}>
                          ...
                          {[1,2,3,4].map(i=>(
                            <div key={i} style={{
                              ...
                              <div style={{
                                ...
                                background: i<=pw.s ? pw.color : "transparent",
                                transformOrigin:"left",
                                animation: i<=pw.s ? "seg-fill .3s ease both" : "none",
                        ...
                        <span style={{
                          fontFamily:"var(--font-body)", fontSize:".66rem", fontWeight:700,
                          letterSpacing:".07em", color:pw.color,
                          textShadow:`0 0 10px ${pw.color}55`,
                        }}>{pw.label}</span>
                      </div>
                    )}
                  </div>
                ...
                <div style={{
                  ...
                  <span style={{
                    ...
                    letterSpacing:".08em", color:"var(--color-text)", opacity:.35,
                  }}>OR</span>
                  <div style={{ flex:1, height:1, background:"rgba(255,255,255,.22)" }}/>
                </div>
                ...
                <p style={{
                  ...
                  color:"var(--color-dark)", textAlign:"center", margin:0, opacity:.75,
                }}>
                  ...
                  <button onClick={()=>navigate("/login")} style={{
                    ...
                    textDecorationColor:"rgba(63,125,88,.3)",
                    transition:"color .18s, text-shadow .18s",
                  }}
                    onMouseEnter={e=>{
                      e.currentTarget.style.color="var(--color-brand)";
                      e.currentTarget.style.textShadow="0 0 12px rgba(63,125,88,.3)";
                    }}
                    onMouseLeave={e=>{
        ...
        <footer style={{
          ...
          <p style={{
            ...
            color:"var(--color-text)", margin:0, opacity:.35,
          }}>
